...
public class Room extends Rectangular {
	...
	public boolean checkCameraIsRoomPoint(Camera camera) {
		for (int i = 0; i < 8; i++) {
			...
			if (camera.getX() == point.getX()*100 && camera.getY() == point.getY()*100 && camera.getZ() == point.getZ()*100) {
				return true;
			}
		}
		return false;
	...
	public boolean checkCameraValid(Camera camera) {
		return camera.getX() > this.getPoints()[0].getX()*100 && camera.getX() < this.getPoints()[1].getX()*100
				&& camera.getY() > this.getPoints()[0].getY()*100 && camera.getY() < this.getPoints()[3].getY()*100
				&& camera.getZ() > this.getPoints()[0].getZ()*100 && camera.getZ() < this.getPoints()[4].getZ()*100
				|| camera.getZ() == this.getPoints()[0].getZ()*100 || checkCameraIsRoomPoint(camera) == true
				|| checkCameraExist(camera) ? false : true;
	}
	
	 public static void createNewRoom(App app) {
	        try {
	            File obj = new File("G:/New folder/bt-lon-java-main/src/input.txt");
	 ...
	 public static int calculateObscuredArea(App app, double x, double y, double z, int print) {
	        if(app.getRooms().size() == 0) {// check cÃ³ room nÃ o chÆ°a
	            return 0;
	        }
	        ...
	        point.setZ(App.round(z));//set tá»�a Ä‘á»™
	        for(int i = 0; i < currentRoom.getObjects().size(); i++) {//check Ä‘iá»ƒm cÃ³ náº±m trong object k
	        ...
	        label: for(int i = 0; i < currentRoom.getCameras().size(); i++) {// loop qua all cam
	            ...
	            for(int j = 0; j < currentRoom.getObjects().size(); j++) {//loop qua all obj
	                ...
	                range[1] = Math.min(lineSegment.getRange()[1], currentObject.getPoints()[1].getX());// cÃ³ khoáº£ng giao nhau giá»¯a Ä‘t vÃ  obj
	                ...
	                switch(countNotZero.size()) {// chia sá»‘ sá»‘ trong vto chá»‰ phÆ°Æ¡ng =0
	                    case 3:
